...
func NewMongoService[DocType interface{}](config MongoServiceConfig) DbService[DocType] {
	...
	if svc.DbName == "" {
		svc.DbName = enviro("AMBULANCE_API_MONGODB_DATABASE", "meetings")
	}

	if svc.Collection == "" {
		svc.Collection = enviro("AMBULANCE_API_MONGODB_COLLECTION", "meetings")
	}
...
func (this *mongoSvc[DocType]) UpdateDocument(ctx context.Context, id string, document *DocType) error {
	...
	// Ensure the id field in the document remains the same
	var updateDoc bson.M
	if err := bson.Unmarshal(docBytes, &updateDoc); err != nil {
		return err
	}
	updateDoc["id"] = id // Ensure the id field remains unchanged

	_, err = collection.ReplaceOne(ctx, bson.D{{Key: "id", Value: id}}, updateDoc)
	return err
}
